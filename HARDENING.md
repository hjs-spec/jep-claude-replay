...
Static and archive UI paths are confined to their configured roots, including symlinks and encoded traversal. serve(..., archive_root=...) explicitly selects an archive directory; only bounded JSONL files are exposed. Ed25519 verification rejects small-order key forgeries using PyNaCl/libsodium. Saved keyrings are atomically replaced with private permissions; rotation requires a new key identifier. Existing archive payload canonicalization and golden hashes remain unchanged.
